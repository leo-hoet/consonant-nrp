...
model.objective = pyo.Objective(rule=objective_rule, sense=pyo.maximize, doc='Objective function without weighting function')


def disponibility_rule_pos(model, alpha):
    return model.p3 - sum(model.e2[j] * model.x_Pos[j, alpha] for j in model.R) >= (alpha - 1) * (model.p4 - model.p3 + sum((model.e2[j] - model.e1[j]) * model.x_Pos[j, alpha] for j in model.R))
...
def disponibility_rule_nec(model, alpha):
    return model.p2 - sum(model.e3[j] * model.x_Nec[j, alpha] for j in model.R) >= (1 - alpha) * (model.p2 - model.p1 + sum((model.e4[j] - model.e3[j]) * model.x_Nec[j, alpha] for j in model.R))
...
def pyomo_postprocess(options=None, instance=None, results=None):
    ...
    for alpha1, alpha2 in alphas2:
        if not NR_Reqs_Nec[alpha1].issubset(NR_Reqs_Nec[alpha2]):
            ...
            print('Los requerimientos seleccionados para el nivel de necesidad {} son {}, y el valor de z correspondiente es {}'.format(1 - alpha1, NR_Reqs_Nec[alpha1], sum(instance.u[i] * pyo.value(instance.y_Nec[i, alpha1]) for i in instance.C)))
    print('Los requerimientos seleccionados para el nivel de necesidad {} son {}, y el valor de z correspondiente es {}'.format(1 - alphas[-1], NR_Reqs_Nec[alphas[-1]], sum(instance.u[i] * pyo.value(instance.y_Nec[i, alphas[-1]]) for i in instance.C)))
    ...
    for alpha1, alpha2 in alphas2:
        if not NR_Reqs_Pos[alpha1].issubset(NR_Reqs_Pos[alpha2]):
            ...
            print('Los requerimientos seleccionados para el nivel de posibilidad {} son {}, y el valor de z correspondiente es {}'.format(alpha1, NR_Reqs_Pos[alpha1], sum(instance.u[i] * pyo.value(instance.y_Pos[i, alpha1]) for i in instance.C)))
    print('Los requerimientos seleccionados para el nivel de posibilidad {} son {}, y el valor de z correspondiente es {}'.format(alphas[-1], NR_Reqs_Pos[alphas[-1]], sum(instance.u[i] * pyo.value(instance.y_Pos[i, alphas[-1]]) for i in instance.C)))
    print("El valor esperado de este plan consonante es {0}".format(pyo.value(instance.objective)))

    nec_r_in_plan = {r: (1 if not (ALPHAS := [alpha for alpha in instance.AC if pyo.value(instance.x_Nec[r, alpha]) == 1]) else min(ALPHAS)) for r in instance.R}
    pos_r_in_plan = {r: (0 if not (ALPHAS := [alpha for alpha in instance.AC if pyo.value(instance.x_Pos[r, alpha]) == 1]) else max(ALPHAS)) for r in instance.R}
    ...
    table_alpha_data = [[alpha, ", ".join(str(r) for r in NR_Reqs_Nec[alpha]), ", ".join(str(r) for r in NR_Reqs_Pos[alpha]),
                         ", ".join(str(c) for c in NR_Custs_Nec[alpha]), ", ".join(str(c) for c in NR_Custs_Pos[alpha])] for alpha in instance.AC]
    ...
    for i, alpha in enumerate(instance.AC):
        ...
        axs[i, 0].plot([E1 := sum(instance.e1[r] for r in instance.R if pyo.value(instance.x_Nec[r, alpha]) == 1),
                  sum(instance.e2[r] for r in instance.R if pyo.value(instance.x_Nec[r, alpha]) == 1),
                  sum(instance.e3[r] for r in instance.R if pyo.value(instance.x_Nec[r, alpha]) == 1),
                  sum(E4 := instance.e4[r] for r in instance.R if pyo.value(instance.x_Nec[r, alpha]) == 1)],
                       [0, 1, 1, 0], label=r"$\sum_{r_j \in R} \tilde e_j \cdot x_{j,{alpha}}^{Nec}$")
        axs[i, 0].plot([min(instance.p1, E1)-10, max(instance.p4, E4)+10], [alpha, alpha], '--')
        ...
        axs[i, 1].plot([E1 := sum(instance.e1[r] for r in instance.R if pyo.value(instance.x_Pos[r, alpha]) == 1),
                        sum(instance.e2[r] for r in instance.R if pyo.value(instance.x_Pos[r, alpha]) == 1),
                        sum(instance.e3[r] for r in instance.R if pyo.value(instance.x_Pos[r, alpha]) == 1),
                        E4 := sum(instance.e4[r] for r in instance.R if pyo.value(instance.x_Pos[r, alpha]) == 1)],
                       [0, 1, 1, 0], label=r"$\sum_{r_j \in R} \tilde e_j \cdot x_{j,{alpha}}^{Pos}$")
        axs[i, 1].plot([min(instance.p1, E1) - 10, max(instance.p4, E4) + 10], [alpha, alpha], '--')
...
if __name__ == '__main__':
    ...
    #instance = model.create_instance("nrp_536c_3502r.dat")
    from pyomo.opt import SolverManagerFactory

    manager = pyo.SolverManagerFactory('neos')
    os.environ['NEOS_EMAIL'] = 'casanovac@frcu.utn.edu.ar'

    # get the start time
    st = time.time()
    results = manager.solve(instance, opt='cplex', options={'mipgap': 0.01})
    #results = manager.solve(instance, opt='cplex')
    # get the end time
    et = time.time()
